...
//		insert, delete, update sql 명령의 실행 결과를 리턴시키면 sql 명령이 성공적으로 실행된
//		횟수가 리턴된다.
...
//	FreeboardCommentService 클래스에서 호출되는 mapper와 공지글 또는 메인글의 글번호를 넘겨받고
//	 댓글 갯수를 세는 freeboardcomment.xml 파일의 select sql 명령을 실행하는 메소드
...
//	FreeboardCommentService 클래스에서 호출되는 mapper와 공지글 또는 메인글의 글번호를 넘겨받고
//	공지글 또는 메인글의 댓글 목록을 얻어오는 freeboardcomment.xml 파일의 select sql 명령을 실행하는 메소드
...
//	FreeboardCommentDAO 클래스에서 호출되는 mapper와 댓글을 삭제할 정보가 저장된 객체를
