...
<%@page import="com.tjoeun.freeboard.vo.FreeboardCommentList"%>
...
//	increment.jsp와 contentView.jsp 넘어오는 글번호와 돌아갈 페이지 번호, 분기할 페이지 이름을 받는다. 	
...
//	브라우저에 출력할 메인글이 저장된 객체, 작업후 돌아갈 페이지 번호, 줄바꿈에 사용할
//	이스케이프 시퀀스("\r\n")를 request 영역에 저장한다.
	request.setAttribute("vo", vo);
	request.setAttribute("currentPage", currentPage);
	request.setAttribute("enter", "\r\n");
	
//	job에 contentView가 넘어왔을경우 댓글 목록을 얻어와서 request 영역에 저장시킨다.
...
//	increment.jsp에서 넘어온 경우 request 영역에 저장된 내용을 가지고 조회수를 증가시킨 글을 
//	브라우저에 출력하는 페이지(contentView.jsp)로 넘겨주고 contentView.jsp에서 넘어온 경우 메인글을 수정(update.jsp).
//	삭제(delete.jsp)하는 페이지로 넘겨준다.
